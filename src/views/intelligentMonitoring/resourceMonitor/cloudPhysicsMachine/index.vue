...
<template>
  <div v-loading="loading" class="monitor cloud-physics-machine">
...
export default {
  ...
  watch: {
    $route(to, from) {
      this.detailFlg =
        this.$route.path.indexOf("cloudPhysicsMachineDetail") > -1;
    },
  ...
  data() {
    return {
      ...
      detailFlg: this.$route.path.indexOf("cloudPhysicsMachineDetail") > -1,
      loading: false,
      tableLoading: false,
      alertStatus: "",
      timeoutID: 0,
  ...
  methods: {
        ...
        case "tag":
          ...
          this.$refs.editTag.show(
            Object.assign(rowData, {
              logDescNameCN: "云物理机",
              logDescNameEN: "Cloud Physical Machine"
            })
          );
          break;
        case "operate":
    ...
    async getTableData(v = {}) {
      ...
      if (me.urlSearchParams && me.urlSearchParams.get("test")) {
        url = "static/test/cloudPhysicsMachine/sub_resources.json";
      }
      ...
      rl.forEach((e, i, a) => {
        me.tableData.push({
          ...
          status: me.getStatusContent(e["status"]),
    ...
    getTagList() {
      ...
      var url = "api/pluto/v1/monitor/resources/tags?subclass_id=cpm" + p;
      this.$ajax({
        type: "get",
        url: url,
        headers: {
    ...
    gotoDetails(v) {
      this.$router.push({
        ...
        params: { hostName: v.collectHost, resourceId: v.resourceID, name: v.name },
        query: { test: v.resourceID }
      });
    },
    getStatusContent(v) {
      v = {
        BUSY: '<i class="el-icon-loading"></i>',
        ACTIVE: this.$t("calcStorLang.running"),
        BUILD: this.$t("calcStorLang.build"),
        ERROR: this.$t("calcStorLang.error"),
        REBOOT: this.$t("calcStorLang.reboot"),
        HARD_REBOOT: this.$t("calcStorLang.hard_reboot"),
        PAUSED: this.$t("calcStorLang.paused"),
        MIGRATING: this.$t("calcStorLang.migrating"),
        PASSWORD: this.$t("calcStorLang.password"),
        REBUILD: this.$t("calcStorLang.rebuild"),
        RESCUE: this.$t("calcStorLang.rescue"),
        REVERT_RESIZE: this.$t("calcStorLang.revert_resize"),
        SHELVED: this.$t("calcStorLang.shelved_offloaded"),
        SHELVED_OFFLOADED: this.$t("calcStorLang.shelved_offloaded"),
        SHUTOFF: this.$t("calcStorLang.closed"),
        SUSPENDED: this.$t("calcStorLang.suspended"),
        UNKNOWN: this.$t("calcStorLang.unknow"),
        VERIFY_RESIZE: this.$t("calcStorLang.verify_resize"),
        CRASHED: this.$t("calcStorLang.crashed"),
        SOFT_DELETED: this.$t("calcStorLang.instDetailSoftDeleted")
      }[v];
      if (!v) {
        return "-";
      }
...
<style scoped>
.cloud-physics-machine >>> .content table tbody tr td:first-child:hover {
  text-decoration: underline;
}
.cloud-physics-machine >>> .content table tbody tr td:first-child,
.cloud-physics-machine >>> .content table tbody tr td:last-child,
.cloud-physics-machine >>> .content table tbody tr td:nth-child(2) {
  cursor: pointer;
  color: #1991eb;
}
</style>