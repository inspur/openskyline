...
export default {
  ...
  data() {
    let legend = [
      Vue.t("monitor.VDISK_MB")
    ];
    return {
      title: `${Vue.t("monitor.VDISK_MB")}(MBps)`,
      loading: false,
  ...
  methods: {
    getData(v = {}) {
      ...
      this.$ajax({
        ...
        data: JSON.stringify({
          params: {
            alias_fields: ['vdisk_mb'],
            fields: ['last(vdisk_mb)'],
            filters: [
              {
                field: "g2name",
                operate: "=",
        ...
        successFun(rtn) {
          ...
          data.forEach((item, index) => {
            ...
            y1.push(item["vdisk_mb"] != null ? item["vdisk_mb"].toFixed(2) : null);
          });
