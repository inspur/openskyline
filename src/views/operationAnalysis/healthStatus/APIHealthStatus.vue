<template>
  <el-collapse v-model="activeNames">
    <el-collapse-item name="item">
      <template slot="title">
        {{ $t('operationAnalysis.HEALTH_STATUS_API') }}
        ...
        <el-table-column type="expand">
          <template slot-scope="scope">
            <el-table :data="scope.row.data">
              <el-table-column :label="$t('operationAnalysis.HEALTH_STATUS_NAME')">
                <template slot-scope="scopeDetail">
                  <div v-if="scopeDetail.row.type !== 'server'">
                    {{ scopeDetail.row.type }}: {{ scopeDetail.row.host }}
                  </div>
                  <div v-else>
                    {{ scopeDetail.row.type }}: {{ scopeDetail.row.sv }}
                  </div>
                </template>
              </el-table-column>
              <el-table-column :label="$t('operationAnalysis.HEALTH_STATUS_STATUS')" prop="status" />
            </el-table>
          </template>
        </el-table-column>
  ...
  methods: {
    async load () {
      ...
      try {
        ...
        $this.$emit('onLoaded', { okCount: res.ok_count, midCount: res.mid_count, errCount: res.err_count, total: res.total });
      } catch (e) {
        $this.error = true;
        $this.loading = false;
    ...
    status (value) {
      switch (value) {
        ...
        case Constants.STATUS_MID:
          return this.$t('operationAnalysis.HEALTH_STATUS_MID');
        case Constants.STATUS_ERR:
          return this.$t('operationAnalysis.HEALTH_STATUS_ERR');
        default:
          return '-';
      }
    }
