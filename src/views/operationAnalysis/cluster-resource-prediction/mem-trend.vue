  ...
  methods: {
    ...
    async loadChartData() {
      ...
      const res = await $this.$ajax({
        type: 'get',
        url: `api/earth/ntd/tp/cluster_trend?return_type=json&type=mem&revolution=${$this.duration}&model=${this.predictionModel.toLowerCase()}`
      });
    ...
    async loadRestTime() {
      ...
      if (!/^[0-9]+$/.test($this.dashboardOption.percent + '')) {
        ...
        if (threshold > 100 || threshold < 1) {
          ...
          const res = await $this.$ajax({
            type: 'get',
            url: `api/earth/ntd/tp/cluster_predict?return_type=json&type=mem&revolution=${$this.duration}&threshold=${$this.dashboardOption.percent}&model=${this.predictionModel.toLowerCase()}`
          })
    ...
    async loadInfo() {
      ...
      // }) || {};
      // $this.duration = res.revolution_mem || '24h';
    },
    async saveDuration() {
      const $this = this;
      const res = await $this.$ajax({
