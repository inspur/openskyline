...
export default {
  ...
  methods: {
    ...
    getHistoryData() {
      ...
      let ret = this.$ajax({
        type: 'get',
        url: "api/venus/v1/custom_config?id=host",
        successFun(data) {
          let hfs = [1, 2, 3, 4];
          if (!data.value) {
            hfs.forEach((v, i) => {
    ...
    async getHistoryChartData(option, value) {
      ...
      if (queryData["program_name"] != '' && queryData["program_name"] != undefined) {
        reqBody += 'program_name=' + queryData["program_name"] + '&';
      }
    ...
    saveHistoryData() {
      ...
      queryData["program_name"] = self.query_zujian;
      queryData["level"] = self.query_level;

      let cn = `${Vue.t("operationAnalysis.hostLogAnalysis")}柱状图TOP5`;
      let en = `${Vue.t("operationAnalysis.hostLogAnalysis")} Log Analysis TOP5`;
      let a = [self.query_module, self.query_zujian, self.query_level];
      if (a[0] || a[1] || a[2]) {
        cn += " (";
        en += " (";
        if (a[0]) { cn += ` ${Vue.t('operationAnalysis.module')}=${a[0]} `; en += ` module=${a[0]} ` };
        if (a[1]) { cn += ` ${Vue.t('operationAnalysis.submodule')}=${a[1]} `; en += ` program=${a[1]} ` };
