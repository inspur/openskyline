<template>
  <div v-loading="loading">
    <el-form :model="formData" :rules="rules" ref="form" onsubmit="return false;" label-width="120px" label-position="right">
      <el-form-item :label="$t('calcStorLang.INSTANCE_ISO_INJECT_IMAGE')">
        <el-row>
          <el-col :span="12">
            <el2-select v-model="formData.imageId" style="width: 100%;">
              <el2-option v-for="(item, index) in images" :key="index" :value="item.id" :label="item.name" />
            </el2-select>
          </el-col>
          <el-col :span="12">
            <el-button :loading="injecting" @click="injectImage()" type="primary" icon="fa-plus" size="small" style="margin-left: 10px;" :disabled="formData.imageId === '' || cdromId !== null">{{ $t('calcStorLang.INSTANCE_ISO_INJECT') }}</el-button>
          </el-col>
        </el-row>
      </el-form-item>
