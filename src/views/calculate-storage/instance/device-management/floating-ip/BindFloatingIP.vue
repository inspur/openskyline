<template>
<div>
  <el2-dialog :title="$t('network.bangdingFloatingip')" :visible.sync="dialogVisible" width="600px" :append-to-body="true" :before-close="handleClose" :close-on-click-modal="false">
    <el-form ref="form" :model="form" label-width="100px" :rules="rules" v-loading="loading">
      <el-form-item :label="$t('network.floatingIP')" class="is-required" prop="floatIp">
        <el2-select v-model="form.floatIp" filterable>
          <el2-option v-for="item in totalFloatingIPs" :key="item.id" :label="item.floating_ip_address" :value="item.id"></el2-option>
        </el2-select>
      ...
      <el-form-item :label="$t('network.fixedipaddress')" class="is-required" prop="fixedIp">
        <el2-select v-model="form.fixedIp">
          <el2-option v-for="item in totalfixedIPs" :key="item.portId" :label="item.fixip" :value="item.portId"></el2-option>
        </el2-select>
        <el2-tooltip placement="top" trigger="hover" :content="$t('calcStorLang.floatIpInstTip')" v-if="this.originFlag == 'sky'">
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="addFloatingIp" :loading="saving">{{$t('lang.confirm')}}</el-button>
    </div>
  </el2-dialog>
  <el2-dialog :title="$t('calcStorLang.instAddFloatingIp')" :visible.sync="addIpFlag" v-if="addIpFlag" width="600px" :append-to-body="true" :before-close="handleAddIpClose" :close-on-click-modal="false">
    <el-form ref="addIp" :model="addIp" label-width="100px" :rules="rules1" v-loading="loading">
      ...
      <el-form-item :label="$t('network.externalNet')" class="is-required" prop="externalnet_id">
        <el2-select class="col-10" v-model="addIp.externalnet_id" filterable clearable @change="getIPAvailabilities">
          <el2-option v-for="item in extNetOption" :key="item.id" :label="item.name" :value="item.id"></el2-option>
        </el2-select>
      </el-form-item>
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" @click="confirmIpFun" :disabled="saving">{{$t('lang.confirm')}}</el-button>
    </div>
 </el2-dialog>
 </div>
</template>
                        ...
                        self.$ajax({
                          ...
                          log:{
                            description:{
                              ...
                              zh_cn:"云主机" + self.instanceName+" 绑定浮动IP:" + ipaddress
                            },
                            target:logtarget
                          }
                        });
