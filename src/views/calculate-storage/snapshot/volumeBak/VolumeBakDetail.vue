                ...
                <table class="table table-content table-hover idatatable" border="1">
                  <tbody>
                    ...
                    <tr class="odd">
                      ...
                      <td class="idatatd">{{detailProject}}</td>
                    </tr>
                    <tr class="odd">
                    ...
                    <tr class="odd">
                      <th class="idatath">{{$t('storage.container')}}：</th>
                      <td class="idatatd">{{detailBak.container}}</td>
                    </tr>
                    <tr class="odd">
              ...
              <table class="table table-content table-hover idatatable" border="1">
                <tbody>
                  <tr class="even">
                    ...
                    <td class="idatatd">{{detailBak.id}}</td>
                  </tr>
                  <tr class="even">
                    <th class="idatath">{{$t('lang.parentid')}}：</th>
                    <td class="idatatd">
                      {{parentidRender(detailBak.parent_id)}}
                    </td>
                  </tr>
                  <tr class="even">
                      <th class="idatath">{{$t('storage.bakType')}}：</th>
                      <td class="idatatd">
                        {{detailBak.is_incremental?$t('storage.increaseNum'):$t('storage.allNum')}}
                      </td>
                    </tr>
                    <tr class="even">
                      <th class="idatath">{{$t('calcStorLang.fvolumeName')}}：</th>
                      <td class="idatatd">
                        {{volumeNameRender(detailBak)}}
                      </td>
                    </tr>
                </tbody>
              </table>
                ...
                <table class="table table-content table-hover idatatable" border="1">
                  <tbody>
                    ...
                    <tr class="odd">
                      ...
                      <td class="idatatd">
                        {{dateRender(detailBak.created_at)}}
                      </td>
                    </tr>
                  </tbody>
                </table>
          ...
          <el-form :inline="true" v-model="formInline" class="">
           <el-row :gutter="20">
             <el-col :span="24">
               ...
               <el-form-item >{{detailBak.fail_reason}}</el-form-item>
             </el-col>
           </el-row>
          </el-form>
        </el-collapse-item>
...
export default {
  name: "detail",
  props: ['detailBak', 'userName', 'vmpName', 'deviceUN', 'detailProject'],
  data() {
    return {
  ...
  methods: {
    ...
    getVolumeName() {
      var volName = this.detailBak.id;
      if (typeof (this.detailBak.name) == "undefined" || null == this.detailBak.name || '' == this.detailBak.name) {
        volName = this.detailBak.id;
      } else {
        volName = this.detailBak.name;
      }
      return volName;
    },
    volumeStatusRender() {
      let self = this;
      let value = self.detailBak.status;
      self.statusList.forEach(item => {
        if (item.id === self.detailBak.status) {
          value = item.name;
    ...
    getVolumeDesc() {
      if (typeof (this.detailBak.description) == "undefined" || null == this.detailBak.description || '' == this.detailBak.description) {
        return '';
      } else {
        return this.detailBak.description;
      }
    },
    buildStatusList() {
      this.statusList = [
        ...
        { id: "restoring", name: this.$t("storage.restoring") }
      ];
    },
    dateRender(value) {
      var self = this;
