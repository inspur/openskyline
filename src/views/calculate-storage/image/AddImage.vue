...
export default {
  ...
  methods: {
      ...
      try {
        let ret = await this.$ajax({
          ...
          log: {
              description: {
                en: "Add image:" + name,
                zh_cn: Vue.t('calcStorLang.addImage') + name
              },
              key: "name",
    ...
    async loadimgs() {
      ...
      //   $this.newImage.core = $this.coreOptions[0].value;
      // }
      // if ($this.ramdiskOptions.length > 0) {
      //   $this.newImage.ramdisk = $this.ramdiskOptions[0].value;
      // }
    },
    async loadProjects() {
      const $this = this;
    ...
    async upload(options) {
      ...
      console.log('上传开始：' + startTime);
      try {
        const res = await $this.$ajax({
          type: 'put',
          url: `api/glance/v2/images/${$this.uploadData.imageUuid}/file`,
          data: file,
          processData: false,
        ...
        console.log('上传成功：' + endTime);
        console.log('耗时：' + ((endTime - startTime) / 1000) + 's');
        options.onSuccess();
      } catch (e) {
        const endTime = (new Date()).getTime();
        console.log('上传失败：' + endTime);
        console.log('耗时：' + ((endTime - startTime) / 1000) + 's');
        options.onError();
      }
    }
