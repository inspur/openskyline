<template>
  <div>
    <el-dialog
      class="select-dialog"
      :title="$t('base.select') + $t('calcStorLang.deployImageMomerydisk')"
      :show-close="!saving"
      ...
      <div>
        ...
        <el-table :data="tableData" @row-click="handleRowClick" @sort-change="handleSortChanged" @row-dblclick="save">
          <el-table-column width="35">
            <template slot-scope="scope">
              <el-radio v-model="selectedImageRamdiskId" :label="scope.row.value">&nbsp;</el-radio>
            </template>
          </el-table-column>
        ...
        <el-row type="flex" justify="space-between" align="middle">
          <icos-column-selector @refresh="loadData" @column-checked="handleColumnChecked" :selected-length="selectedImageRamdiskId === '' ? 0 : 1" :columns="[
          {
            prop: 'label',
            label: $t('lang.name'),
            visible: true,
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="save" :disabled="selectedImageRamdiskId === ''" :loading="saving">{{ $t('base.confirm') }}</el-button>
      </span>
    </el-dialog>
   </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async save() {
      const $this = this;
      const selectedImageRamdisk = $this.imageRamdisks.find(item => item.value === $this.selectedImageRamdiskId);
      $this.$emit('select', selectedImageRamdisk);
      $this.close();
    },
    loadData() {
      const $this = this;
      ...
      $this.totalData = $this.imageRamdisks.filter(item => item.label && item.label.toLowerCase().includes(searchName));
      $this.selectedImageRamdiskId = '';
    },
    ...
    handleRowClick(row) {
      this.selectedImageRamdiskId = row.value;
    },
