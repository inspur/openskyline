    ...
    <el-collapse v-model="activeNames">
      ...
      <el-collapse-item :title="$t('base.performanceCurve')+'('+timeTitle+')'" name="2" style="text-align: left;">
        ...
        <el-row>
          <el-col :span="12">
            <div style="height: 200px;">
              <IEcharts ref="" :option="option1" style="height:100%;"></IEcharts>
            </div>
            <div style="text-align: center;line-height: 36px;">{{$t('base.cpuUsHisCur')}}</div>
          </el-col>
          <el-col :span="12">
            <div style="height: 200px;">
              <IEcharts ref="" :option="option2" style="height:100%;"></IEcharts>
            </div>
        ...
        <el-row>
          <el-col :span="12">
            <div style="height: 200px;">
              <IEcharts ref="diskChart" :option="option3" style="height:100%;"></IEcharts>
            </div>
            <div style="text-align: center;line-height: 36px;">{{$t('base.diskRWHisCur')}}</div>
          </el-col>
          <el-col :span="12">
            <div style="height: 200px;">
              <IEcharts ref="netChart" :option="option4" style="height:100%;"></IEcharts>
            </div>
  ...
  methods: {
    ...
    getNetwork(arr) {
      ...
      Promise.all(arr).then(function(data) {
        ...
        me.setOption('option4', data[0], Vue.t('base.in'), "bytes_recv");
        me.setOption('option4', data[1], Vue.t('base.out'), "bytes_sent");
      })
    },
    ...
    async getRealTimeData() {
      ...
      this.realTimeData = {
        ...
        diskin:obj.diskin?obj.diskin.toFixed(2):obj.diskin,
        diskout:obj.diskout?obj.diskout.toFixed(2):obj.diskout,
        mem_utilzation:(obj.mem_utilzation===''||isNaN(obj.mem_utilzation))?obj.mem_utilzation:obj.mem_utilzation.toFixed(2)+"%",
        netin:obj.netin?obj.netin.toFixed(2):obj.netin,
        netout:obj.netout?obj.netout.toFixed(2):obj.netout
      }
    ...
    async getCpuHisData(timeStr) {
      let obj = await this.$ajax({
        type: 'get',
        polling:true,
        url: "api/pluto/v1/monitor/hosts/"+this.hostname+"/history?return_type=json&type=cpu"+timeStr
      })
      let cpuArr = obj.cpu||[];
      let x = [];
      let y = [];
      cpuArr.forEach((item, index) => {
        ...
        if (item[1]&&item[1]["used"]!=null) {
          y.push(item[1]["used"].toFixed(2));
        } else {
          y.push(null);
        }
      })
    ...
    async getMemHisData(timeStr) {
      let obj = await this.$ajax({
        type: 'get',
        polling:true,
        url: "api/pluto/v1/monitor/hosts/"+this.hostname+"/history?return_type=json&type=mem"+timeStr
      })
      let memArr = obj.mem||[];
      let x = [];
      let y = [];
      memArr.forEach((item, index) => {
        x.push(this.convertX(item[0].time));
        if (item[1]&&item[1]["used_percent"]) {
          y.push(item[1]["used_percent"].toFixed(2));
        } else {
          y.push(null);
        }
      })
    ...
    async getDiskHisData(timeStr) {
      let disk = await this.$ajax({
        type: 'get',
        url: "api/pluto/v1/monitor/hosts/"+this.hostname+"/history?return_type=json&type=disk"+timeStr
      })
      let x = [];
      let me = this;
      let flg = true;
      ...
      for (let key in disk) {
        ...
        arr.forEach((item, index) => {
          ...
          if (item[1]&&item[1].read_bytes!=null) {
            yread.push(item[1].read_bytes.toFixed(2));
          } else {
            yread.push(null);
          }
          if (item[2]&&item[2].write_bytes!=null) {
            ywrite.push(item[2].write_bytes.toFixed(2));
          } else {
            ywrite.push(null);
          }
          if (index==arr.length-1) {
    ...
    async getNetinData(timeStr) {
      return new Promise((resolve, reject) => {
        this.$ajax({
          type: 'get',
        polling:true,
        url: "api/pluto/v1/monitor/hosts/"+this.hostname+"/history?return_type=json&type=netin"+timeStr,
          successFun(data) {
            resolve(data)
          },
          errFun() {
    ...
    async getNetoutData(timeStr) {
      return new Promise((resolve, reject) => {
        this.$ajax({
          type: 'get',
          polling:true,
          url: "api/pluto/v1/monitor/hosts/"+this.hostname+"/history?return_type=json&type=netout"+timeStr,
            successFun(data) {
              resolve(data)
            },
            errFun() {
...
<style scoped>
  .performance-monitoring /deep/ .el-collapse-item__wrap{
    overflow:visible;
  }
</style>