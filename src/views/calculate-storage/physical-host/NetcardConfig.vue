<template>
  <div>
    <el-dialog :title="$t('calcStorLang.instNetcardCofig')" :visible.sync="dialogVisible" :before-close="handleClose">
      <div class="operation-panel">
        <operation-panel ref="operationOperate" :select-rows="multipleSelection" :operation-menus="operationMenus" enableFlgstyle="display:inline;">
        </operation-panel>
      </div>
    ...
    <add-netcard v-if="addFlag" @onRefresh="onRefresh" @addhideEdit="addFlag=false" :hostEntity="hostEntity"></add-netcard>
    <qos v-if="qosConfigFlag" @onRefresh="onRefresh" @hideQos="qosConfigFlag=false" :hostEntity="hostEntity" :selectRows="selectRows"></qos>
  </div>
</template>
<script>
import AddNetcard from './AddNetcard';
import CardOperateInstance from '../instance/CardOperateInstance';
import Qos from '../instance/QosConfig';
export default {
  name: 'netcard',
  props: ['hostEntity'],
  components: {AddNetcard, Qos, CardOperateInstance},
  data() {
    return {
      currentPage: 1,
      addFlag: false,
      ...
      multipleSelection: [],
      multipleSelection2: [],
      operationMenus: CardOperateInstance.bind(this)(),
      dialogVisible: false,
      totalData: [],
      loading: false,
      ipversion:4
