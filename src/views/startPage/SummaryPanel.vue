  ...
  <div :class="'wrapper_start' + theme">
    ...
    <div class="detail">
      <div :class="'health-status'+theme">
        ...
        <span v-if="!error && !loading" :class="'health-status-ok'+theme">
          {{$t('operationAnalysis.HEALTH_STATUS_OK')}}:
          <span>{{okCount}}</span>
        </span>
        <span v-if="!error && !loading" :class="'health-status-not-ok'+theme">
...
.health-status-ok-dark {
  border-right: 1px solid #eee;
}
.health-status-ok-dark,
.health-status-not-ok-dark {
  flex: 1;
  display: flex;
  align-items: center;
  justify-content: center;
...
.health-status-ok-dark span,
.health-status-not-ok-dark span {
  color: #9c9c9c;
}
</style>
