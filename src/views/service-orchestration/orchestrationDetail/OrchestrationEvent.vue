...
<!--         <template slot-scope="scope">
  <el-button @click="resourceIdRender(scope.$index, scope.row)" type="text" size="small">{{scope.row.resourceId}}</el-button>
      ...
      <el-table-column
        v-if="columnsChecked.indexOf('event_time') >= 0"
        prop="event_time"
        :label="$t('heat.eventTime')"
        show-overflow-tooltip
        align="left"
        sortable="custom"
        min-width="100">
        <template slot-scope="scope">
           <span>{{tzTimeFormatter(scope.row.event_time)}}</span>
        </template>
      </el-table-column>
      <el-table-column
  ...
  export default {
    ...
    data () {
      return {
        ...
        columns: [],
        totalData:[],
        columnsChecked:["resource_name", "physical_resource_id", "event_time", "resource_status", "resource_status_reason"],
        tableData: [],
        loading: false,
        queryParam: {
          field: "",
       ...
       async getData(param) {
        ...
        try {
          let ret = await this.$ajax({
            type: 'get',
            url: "api/heat/v1/" + this.$cookie.get("pid") + "/stacks/" + this.stackName + "/" + this.stackId +"/events"
          });
          self.loading = false;

          let projects = ret.events;
