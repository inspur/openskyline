<template>
  <el-dialog :title="$t('base.add')" :visible.sync="visible">
    <el-form ref="newLeaseForm" label-width="100px" :model="leaseObj" :rules="rules" v-loading="loading" :element-loading-text="$t('base.sendingData')">
...
export default {
  ...
  methods:{
    ...
    confirmFun() {
      let me = this;
      this.$refs.newLeaseForm.validate((valid) => {
        ...
        let ret = this.$ajax({
          ...
          data:JSON.stringify({
            param:{
              "duration":this.leaseObj.duration+"",
          ...
          log:{
            description:{
              en:"long term parameters of new lease term",
              zh_cn:"新建租期时长参数"
            },
