...
var DEFAULT_URL = 'Help.pdf';
var icosCookies = document.cookie.split(';');
var icosLan = "zh-CN";
icosCookies.forEach(function(item) {
  if (item.indexOf('defaultLanguage') > -1) {
    if (item.split('=')[1] == "en") icosLan = "en";
  }
});
...
let arch = getQueryVariable("arch");

var roleType = "";
icosCookies.forEach(function(item) {
  if (item.indexOf('roleType') > -1) {
    roleType = item.split('=')[1];
  }
});
if (roleType == "0") {
  if (arch == "x86") {
    DEFAULT_URL = "SuperAdmin.pdf";
  } else {
    DEFAULT_URL = "SuperAdmin_AK.pdf";
  }
} else if (roleType == "2") {
  if (arch == "x86") {
    DEFAULT_URL = "ProjectAdmin.pdf";
  } else {
    DEFAULT_URL = "ProjectAdmin_AK.pdf";
  }
} else if (roleType == "3") {
  if (arch == "x86") {
    DEFAULT_URL = "ProjectUser.pdf";
  } else {
    DEFAULT_URL = "ProjectUser_AK.pdf";
  }
}

PDFJS.locale = icosLan;
var pdfjsWebLibs = {
